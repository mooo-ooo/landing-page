...
const exchangesHasFundingRate: string[] = ["coinex", "bybit", "okx"];

type IFundingRate = Record<
  string,
  Record<string, { symbol: string; rate: number; interval: number | null }>
>;

export const useFundingRates = () => {
  const positionsStore = useSelector(selectPositions);
  const [fundingRates, setFundingRates] = useState<IFundingRate>({});
      ...
      try {
        ...
        Object.keys(positionsStore).forEach((exchangeName) => {
          const exchangePositions =
            positionsStore[exchangeName as keyof PostitionsState];
        ...
        const uniqueFundingRates = fundingRateRequireds.filter(
          (item, index, self) =>
            ...
            self.findIndex(
              (t) =>
                t.exchange === item.exchange && t.baseToken === item.baseToken
            )
        );
...
export const useSharedFundingRates = (positionsStore?: PostitionsState) => {
  const [fundingRates, setFundingRates] = useState<IFundingRate>({});
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    if (positionsStore) {
