...
export const ExchangeMarginMobile = () => {
  const { enqueueSnackbar } = useSnackbar();
  const error = useSelector(selectBalancesError);
  const balances = useSelector(selectBalances);
  const {exchangeLeverages} = useSelector(selectGroup);
                        ...
                        >
                          <Box>
                            <Box display='flex' justifyContent='space-between' mt={1}>
                              <Typography color="textSecondary" my={1}>Equity</Typography>
                              <Typography color="textSecondary" my={1}>${numeral(exchange.marginBalance).format("0,0")}</Typography>
                            </Box>
                            <Box display='flex' justifyContent='space-between'>
                              <Typography color="textSecondary" my={1}>Available</Typography>
                              <Typography color="textSecondary" my={1}>${numeral(exchange.marginAvailable).format("0,0")}</Typography>
                            </Box>
                            <Box display='flex' justifyContent='space-between' my={1}>
                              <Typography color="textSecondary" my={1}>Spot assets</Typography>
                              <Button onClick={() => handleSubmit(exchangeName.toLowerCase())} size="small" variant="outlined" color="info">USD-M</Button>
